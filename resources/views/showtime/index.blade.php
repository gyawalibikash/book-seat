            ...
            <div class="col-md-6 col-md-offset-1">
                ...
                <table class="table" style="border:2px solid white;box-shadow:4px 4px 2px rgba(0,0,0,0.2)">
                    ...
                    @foreach($showtimes as $showtime)
                        <tr><td style="font-size:30px;">{{ $showtime->time }}</td><td><a href="{{ action('BookSeatController@getMovieshow','?'.http_build_query(['movie'=>$movie->id, 'cinehall'=>$cinehall->id, 'hall'=>$hall->id, 'showtime'=>$showtime->id])) }}" class="btn btn-success btn-lg"><i class="glyphicon glyphicon-facetime-video" ></i></a></td>
                    @endforeach
                </table>
            </div>
